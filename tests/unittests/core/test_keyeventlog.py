...
class TestKeyEventExtraBlocksAndMempoolCoverage(AsyncTestCase):
    """Cover extra_blocks / use_mempool branches added for chain sync."""
    ...
    async def test_verify_confirming_mempool_parent_returns(self):
        from yadacoin.core.keyeventlog import KeyEventFlag
        ...
        with patch("yadacoin.core.keyeventlog.P2PKHBitcoinAddress") as mock_btc:
            ...
            await ke.verify(use_mempool=True)

    async def test_verify_confirming_extra_blocks_parent_returns(self):
        from yadacoin.core.keyeventlog import KeyEventFlag
    ...
    async def test_verify_unconfirmed_extra_blocks_parent_returns(self):
        from yadacoin.core.keyeventlog import KeyEventFlag
        ...
        with patch(
            ...
            with patch.object(
                ...
                try:
                    ...
                    pass
        self.assertIn("2.2", paths)

    async def test_init_async_step2_5_extra_blocks_parent(self):
        from yadacoin.core.keyeventlog import (
            KeyEvent,
            KeyEventChainStatus,
            KeyEventFlag,
...
class TestKeyEventSendsAndParent(AsyncTestCase):
    ...
    async def test_get_onchain_parent_coinbase_classified_unconfirmed(self):
        """Multi-output KEL tips are UNCONFIRMED (not confirming shape)."""
        from unittest.mock import AsyncMock, MagicMock, patch
    ...
    async def test_get_onchain_parent_single_output_coinbase_unconfirmed(self):
        """Single-output coinbase reward tip is UNCONFIRMED via is_coinbase."""
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        pub = _INCEPTION_TXN_DICT["public_key"]
        block_addr = str(P2PKHBitcoinAddress.from_pubkey(bytes.fromhex(pub)))
        coinbase_doc = {
            "public_key": pub,
            "transactions": {
                **dict(_INCEPTION_TXN_DICT),
                ...
                "relationship": "",
                "relationship_hash": "",
                "public_key_hash": block_addr,
                "prerotated_key_hash": block_addr,
                "inputs": [],
                "outputs": [{"to": block_addr, "value": 12.5}],
            },
        }
        ...
        self.assertTrue(result["key_event"].txn.coinbase)
        self.assertEqual(result["key_event"].flag, KeyEventFlag.UNCONFIRMED)

    async def test_get_onchain_parent_value_bearing_single_output_confirming_if_not_coinbase(
        self,
    ):
        """Value-bearing confirming spends are CONFIRMING when not a coinbase."""
        from unittest.mock import AsyncMock, MagicMock, patch

        from yadacoin.core.keyeventlog import KeyEventFlag

        # Block public_key differs from txn public_key => not coinbase
        doc = {
            "public_key": _VALID_PUBKEY_PKR2,
            "transactions": {
                **dict(_INCEPTION_TXN_DICT),
                "prev_public_key_hash": _VALID_ADDR_PKR2,
                "relationship": "",
                "relationship_hash": "",
                "prerotated_key_hash": _VALID_ADDR_PKR2,
...
class TestKeyEventLogCoverage100(AsyncTestCase):
    ...
    async def test_kel_spend_conflict_same_public_key(self):
        from yadacoin.core.keyeventlog import KeyEventLog

        self.assertTrue(await KeyEventLog.kel_spend_conflict("pk1", "pk1"))

    async def test_kel_spend_conflict_matching_inception_tags(self):
        from yadacoin.core.keyeventlog import KeyEventLog

        # Tags apply only when on-chain resolution cannot prove same/distinct.
        with patch.object(
            KeyEventLog, "get_inception", new=AsyncMock(return_value=None)
        ):
            self.assertTrue(
                await KeyEventLog.kel_spend_conflict(
                    "aa" * 33,
                    "bb" * 33,
                    inception_a="pool_inc",
                    inception_b="pool_inc",
                )
            )

    async def test_kel_spend_conflict_mismatched_inception_tags(self):
        from yadacoin.core.keyeventlog import KeyEventLog
    ...
    async def test_kel_spend_conflict_onchain_resolution_overrides_tags(self):
        """Resolved inceptions win over forged mismatched tags."""
        from yadacoin.core.keyeventlog import KeyEventLog
        ...
        with patch.object(
            KeyEventLog, "get_inception", new=AsyncMock(return_value=Inc("same"))
        ):
            with patch(
                ...
                self.assertTrue(
                    await KeyEventLog.kel_spend_conflict(
                        "aa" * 33,
                        "bb" * 33,
                        inception_a="forged_a",
                        inception_b="forged_b",
                    )
                )

    async def test_kel_spend_conflict_falls_back_to_onchain_inception(self):
        from yadacoin.core.keyeventlog import KeyEventLog
        ...
        with patch.object(
            KeyEventLog,
            "get_inception",
            new=AsyncMock(side_effect=[Inc("I"), Inc("I")]),
        ):
            with patch(
                "yadacoin.core.keyeventlog.P2PKHBitcoinAddress.from_pubkey",
                side_effect=lambda b: type("A", (), {"__str__": lambda s: "addr"})(),
    ...
    async def test_get_inception_fast_path_pubkey_mismatch_still_returns(self):
        """Pubkey mismatch no longer discards; fast path returns tagged inception."""
        from yadacoin.core.keyeventlog import KeyEventLog
        ...
        result = await KeyEventLog.get_inception(public_key=_VALID_PUBKEY)
        self.assertIsNotNone(result)
        self.assertEqual(result.public_key, _VALID_PUBKEY_B)
...
class TestClassifyKeyEventFlag(unittest.TestCase):
    ...
    def test_value_bearing_single_output_to_prerotated_is_confirming(self):
        from yadacoin.core.keyeventlog import (
            KeyEventFlag,
            classify_key_event_flag,
            looks_like_confirming_key_event,
