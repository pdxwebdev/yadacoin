...
class Consensus(object):
    ...
    def integrate_block_with_existing_chain(self, block, extra_blocks=None):
        ...
        if block.index >= 38144 and (int(block.time) - int(last_block.time)) > target_block_time:
            target_factor = (int(block.time) - int(last_block.time)) / target_block_time
            target = target * (target_factor * 4)
            if target > target_block_time:
                target = self.lowest
        if ((int(block.hash, 16) < target) or 
            (block.special_min and block.index < 35200) or 
            (block.index >= 35200 and block.index < 38144 and block.special_min and (int(block.time) - int(last_block.time)) > target_block_time)):
