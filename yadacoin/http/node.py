...
class GetLatestBlockHandler(BaseHandler):
    ...
    async def get(self):
        ...
        :return:
        """
        block = await self.config.BU.get_latest_block()
...
class GetPeersHandler(BaseHandler):
    ...
    async def get(self):
        ...
        :return:
        """
        self.render_as_json({'peers': [], 'error': 'please update to version 4.0 https://yadacoin.io'})
    ...
    async def post(self):
        ...
        try:
            ...
            if block_data['index'] == 0:
                return
            if int(block_data['version']) != self.config.BU.get_version_for_height(block_data['index']):
