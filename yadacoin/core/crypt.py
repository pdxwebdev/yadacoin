...
class RIPEMD160:
    ...
    def compress(h0, h1, h2, h3, h4, block):
        ...
        for j in range(80):
            ...
            al = RIPEMD160.rol(al + RIPEMD160.fi(bl, cl, dl, rnd) + x[RIPEMD160.ML[j]] + RIPEMD160.KL[rnd], RIPEMD160.RL[j]) + el
            al, bl, cl, dl, el = el, al, bl, RIPEMD160.rol(cl, 10), dl
            # Perform right side of the transformation.
            ar = RIPEMD160.rol(ar + RIPEMD160.fi(br, cr, dr, 4 - rnd) + x[RIPEMD160.MR[j]] + RIPEMD160.KR[rnd], RIPEMD160.RR[j]) + er
