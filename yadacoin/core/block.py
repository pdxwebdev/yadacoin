...
class Block(object):
    ...
    async def init_async(
        ...
        self.mongo = yadacoin.core.config.CONFIG.mongo
    ...
    async def generate(
        ...
        mongo = yadacoin.core.config.CONFIG.mongo
    ...
    async def save(self):
        ...
        res = await self.mongo.async_db.blocks.find_one({"index": (int(self.index) - 1)})
        if (res and res[0]['hash'] == self.prev_hash) or self.index == 0:
            await self.mongo.async_db.blocks.replace_one({'index': self.index}, self.to_dict())
        else:
