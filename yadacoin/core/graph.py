...
class Graph(object):
    ...
    async def update_collection_last_activity(self):
        last_collection_time = (
            await self.config.mongo.async_db.user_collection_last_activity.find_one(
                {
        ...
        self.last_collection_time = (
            last_collection_time["time"] if last_collection_time else 0
        )
        ...
        for rid in self.rids:
            await self.config.mongo.async_db.user_collection_last_activity.update_one(
                {"username_signature": self.username_signature, "rid": rid},
                {
                    "$set": {
                        "username_signature": self.username_signature,
                        "rid": rid,
    ...
    async def get_sent_friend_requests(self, search_rid):
        self.sent_friend_requests = []
        self.sent_friend_requests += [
            x for x in GU().get_sent_friend_requests(search_rid)
        ]
    ...
    async def get_messages(self, not_mine=False):
        if self.wallet_mode:
            for transaction in self.mongo.db.miner_transactions.find(
                {"relationship": {"$ne": ""}}
            ):
                try:
    ...
    async def get_new_messages(self):
        ...
        self.messages = sorted(
            self.messages, key=lambda x: int(x.get("time", 0)), reverse=True
        )
        used_rids = []
    ...
    def get_group_messages(self):
        if self.wallet_mode:
            ...
            blocked = [
                x["username"]
                for x in self.mongo.db.blocked_users.find(
                    {"username_signature": self.username_signature}
                )
            ]
            flagged = [
                x["id"]
                for x in self.mongo.db.flagged_content.find(
                    {"username_signature": self.username_signature}
                )
            ]
            for x in GU().get_posts(self.rid):
                rids = sorted(
                    [str(my_username_signature), str(x.get("username_signature"))],
                    key=str.lower,
                )
                rid = (
                    hashlib.sha256((str(rids[0]) + str(rids[1])).encode("utf-8"))
                    .digest()
                    .hex()
                )
