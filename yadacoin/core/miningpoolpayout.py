...
from yadacoin.core.transaction import Transaction, Input, Output, NotEnoughMoneyException
...
class PoolPayer(object):
    ...
    async def get_share_list_for_height(self, index):
        raw_shares = []
        async for x in self.config.mongo.async_db.shares.find({'index': index, 'address': {'$ne': None}}).sort([('index', 1)]):
            raw_shares.append(x)
        if not raw_shares:
            return False
