...
class Config:
    ...
    def __init__(self, config=None):
        ...
        self.payout_scheme = config.get("payout_scheme", "pplns") # payment calculation system, PPLNS by default, can be changed to PROP
        self.payout_frequency = config.get("payout_frequency", 6) # unused
        self.block_confirmation = config.get("block_confirmation", 6) # the number of confirmations required to pay out a block
        self.expected_share_time = config.get("expected_share_time", 25) # expected share time in seconds. For example: if you set 25, the target difficulty for the miner will be set to reach one share every 25 seconds.
        ...
        self.pool_payer_wait = config.get("pool_payer_wait", 1800) # payout frequency for miners, 30 minutes by default
    ...
    def from_dict(cls, config):
        ...
        cls.payout_scheme = config.get("payout_scheme", "pplns")
        cls.payout_frequency = config.get("payout_frequency", 6)
        cls.block_confirmation = config.get("block_confirmation", 6)
        cls.expected_share_time = config.get("expected_share_time", 25)
