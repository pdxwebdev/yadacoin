...
class MiningPool(object):
        ...
        if not smart_contracts_disabled:
            async for txn in (
                ...
                if (
                    ...
                    in mempool_smart_contract_objs
    ...
    async def verify_pending_transaction(
        ...
        try:
            ...
            # Coinbase transactions are generated by the block builder and must
            # never appear in the mempool.  Reject them explicitly to prevent
            # Transaction.verify()'s early-return coinbase path from being
            # exploited to bypass input/output balance checks.
            if transaction_obj.coinbase:
                self.config.app_log.warning(
                    f"Rejecting coinbase transaction from mempool: {transaction_obj.transaction_signature}"
                )
                return
