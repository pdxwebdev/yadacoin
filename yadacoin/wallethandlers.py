...
class GenerateChildWalletHandler(BaseHandler):
    ...
    async def post(self):
        ...
        last_child_key = self.config.async_db.child_keys.find_one({'account': self.get_body_argument('account')}).sort([('inc', -1)])
        if last_child_key:
        ...
        await self.config.async_db.child_keys.insert_one({
