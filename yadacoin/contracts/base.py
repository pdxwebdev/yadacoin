    ...
    async def verify_generation(self, block, generated_txn=None, mempool_txns=None):
        ...
        try:
            result = verify_signature(
                ...
                hashlib.sha256(generated_txn.transaction_signature.encode()).hexdigest().encode('utf-8'),
                bytes.fromhex(block.public_key)
            )
            if not result:
                raise Exception("Payout miner signature did not verify")
        except:
            raise InvalidTransactionSignatureException("Payout miner signature did not verify")
