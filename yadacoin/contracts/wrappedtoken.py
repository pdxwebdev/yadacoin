...
# DISABLED: TraderPayout - never imported or instantiated from app.py (attack surface reduction)
# class TraderPayout:
#     def __init__(
...
# DISABLED: WrappedTokenContract - never imported or dispatched to from app.py (attack surface reduction)
...
#         )
#
#         return await self.generate_transaction(
#             contract_txn, trigger_txn
#         )
#
...
#                 bytes.fromhex(self.identity.public_key)
#             )
#         )
#         return_address = str(
#             P2PKHBitcoinAddress.from_pubkey(
#                 bytes.fromhex(contract_txn.public_key)
#             )
#         )
...
#             fee=0,
#             outputs=[
#                 Output(to=return_address, value=value_sent_to_address)
#             ],
...
#         payout_txn.transaction_signature = (
#             TU.generate_signature_with_private_key(
#                 binascii.hexlify(base58.b58decode(self.identity.wif))[
#                     2:-10
#                 ].decode(),
#                 payout_txn.hash,
#             )
#         )
#         payout_txn.miner_signature = (
#             TU.generate_signature_with_private_key(
#                 self.config.private_key,
#                 hashlib.sha256(
#                     payout_txn.transaction_signature.encode()
#                 ).hexdigest(),
#             )
#         )
#         return payout_txn
