...
def get_all_forwarders(interface=None):
    try:
        ...
        if len(dns_forwarders) == 0:
            raise NoForwardersConfigured(
                "No Forwarders have been configured for {int}".format(int=interface)
            )
...
def get_active_redirect_record_for_host(host):
    ...
    return {"redirect_host": "default", "active": True}
...
def get_redirect_hostname_for_host(host):
    ...
    if not redirect_hostname:
        raise Exception(
            "Active record, but no hostname redirection for {host}".format(host=host)
        )
...
class UDPServer(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, address):
        try:
            ...
            client_net = IPv4Network("{ip}/24".format(ip=address[0]), strict=False)

            # Search for an interface address on the same network as the client
            for addr in get_my_addresses():
                try:
                    if IPv4Address("{ip}".format(ip=addr)) in client_net:
                        interface = addr
...
def move_address_to_another_network(address, network, netmask):
    address = ip_address(address.encode("utf-16", errors="surrogatepass"))
    target_network = IPv4Network(
        "{ip}/{netmask}".format(ip=network, netmask=netmask), strict=False
    )
...
class DNSQuery(object):
    def __init__(self, data, client_address=None, interface=None):
        self.data = data
        ...
        self.interface = interface if interface is not None else "default"
        self.message = ""
        self._ip = None
        self.error = ""
    ...
    @ip.setter
    def ip(self, ip):
        self._ip = IPv4Address("{ip}".format(ip=ip))

    def resolve(self):
        name = self.question.name
        self.message = "DNS ({dns}): {name}: ?.?.?.?. ".format(
            dns=self.interface, name=self.question.name
        )

        # Handle reverse lookups
        if ".in-addr.arpa." in name:
            # TODO: Can we not just forward these?  No will return host not IP?
            self.error = "Cannot handle reverse lookups yet!"
        ...
        try:
            ...
            # Forward request
            self.message += "Forwarding request. "
        ...
        self.message = self.message.replace("?.?.?.?", str(ip))

        return encoded

    def _resolve_request_locally(self, redirect_host):
        redirection = redirect_host["redirect_host"]

        if redirection.lower() == "default":
            if (
                self.interface == "0.0.0.0"
            ):  # This string is the DEFAULT_INTERFACE constant of DNSServer object!
                self.error = "Cannot resolve default as client interface could not be determined!"
                return self._bad_reply()

            redirection = self.interface
            self.message += "Redirecting to default address. ({address}) ".format(
                address=redirection
            )
            ...
            if (
                ...
                self.error = "Cannot resolve {redirection} as client interface could not be determined!".format(
                    redirection=redirection
                )
                return self._bad_reply()

            address, netmask = redirection.split("/")
            redirection = move_address_to_another_network(
                address=address, network=self.interface, netmask=netmask
            )
            self.message += "Redirecting to {address}. ".format(address=redirection)

        # Check whether we already have an IP (A record)
        # Note: For now we only support IPv4
        try:
            IPv4Address("{ip}".format(ip=redirection))
        ...
        self.message += "Redirecting to {redirection} ".format(redirection=redirection)
        ...
        answer = dns.rrset.from_text(
            str(self.decoded.question[0].name),  # name
            600,  # ttl
    ...
    def _forward_request(self, name):
        try:
    ...
    def _resolve_name_using_socket(self, name):
        ...
        try:
            ...
            raise DNSQueryFailed(
                "Resolve name ({name}) using socket failed: {err} ".format(
                    name=name, err=err
                )
            )
    ...
    def _resolve_name_using_dns_resolver(self, name):
        try:
            network = IPv4Network("{ip}/24".format(ip=self.interface), strict=False)
        ...
        try:
            ...
            self.message += "(dns.resolver failed). "
            raise DNSQueryFailed("dns.resolver failed: {err}".format(err=err))
